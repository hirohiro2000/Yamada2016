	...
	SubShader{
		...
		Pass{
			...
			float4 frag(VertexOutput i) : COLOR{
				...
				// Specular:
				float NdotL = max(0, dot(lightDirection, normalDirection ));
				float LdotH = max(0, dot(lightDirection, halfDirection   ));
				float3 diffuseColor = (tex2D(_MainTex, TRANSFORM_TEX(i.uv0, _MainTex)).rgb*_Color.rgb);
